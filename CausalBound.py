...
class CausalBound(object):
    ...
    def KL_GMM(self, f_weights, g_weights, f_means, g_means, f_stds, g_stds):
        sum_result = 0
        self_KL_GMM_val = self.Self_KL_GMM(f_weights, f_weights, f_means, f_means, f_stds, f_stds)
        N_obs = len(f_weights)
        N_do = len(g_weights)
    ...
    def Solve_Optimization(self, C, cls_size, iter_opt, opt_mode):
        ...
        for iter_idx in range(1,iter_opt):
            ...
            if opt_mode == 'min':
                prev_mu_list.append(curr_mu_solver.x - (1/(cls_size*iter_idx)) *np.random.random(cls_size))
            else:
                prev_mu_list.append(curr_mu_solver.x + (1 / (cls_size* iter_idx)) * np.random.random(cls_size))
