...
class stochExp:
    ...
    calibrate: True implies use calibrated classifiers otherwise base scikit learn classifiers
    ...
    def __init__(self, Xdata, Rdata, K, T0=50, C1=2.0, C2=1.0, isMoM=True, calibrate=False, bsize_mult=7,
                 initial_model=[3, 3, 1], model_ratio=[3, 1], max_depths=[3, 6, 10], n_estimators=[50, 100, 200],
                 colsample_bytrees=[0.6, 0.8], nthread=16, penalty=['l1', 'l2'], C=[1e-3, 1e-2, 1e-1, 1.0, 1e1, 1e2],
                 log_file='rewards_tn.npy', cl_limit=2.0):
        self.T0 = max([3 * K, T0])
        self.Xdata = Xdata
        self.Rdata = Rdata
        self.K = K
    ...
    def modelfit(self, Xtrain, Ytrain, weights=None, cvbreak=None, max_depths=[3, 6, 10], n_estimators=[50, 100, 200],
                 colsample_bytrees=[0.6, 0.8], nthread=4, calibrate=True, cv=False):
        n, m = Xtrain.shape
        if weights is None:
        ...
        grid = {"max_depth": max_depths[0], "colsample_bytree": colsample_bytrees[0], "n_estimators": n_estimators[0],
    ...
    def modelfit_logistic(self, Xtrain, Ytrain, weights=None, cvbreak=None, penalty='l2', C=1.0, nthread=4,
                          calibrate=True):
        n, m = Xtrain.shape
        if weights is None:
            weights = np.ones(len(Ytrain))
    ...
    def bootstrap(self, Xtrain, Ytrain, weights=None, cvbreak=None, max_depths=[3, 6, 10], n_estimators=[50, 100, 200],
                  colsample_bytrees=[0.6, 0.8], nthread=4, calibrate=True, cv=False, Nboot=100):
        L = [0] * Nboot
    ...
    def bootstrap_logistic(self, Xtrain, Ytrain, weights=None, cvbreak=None, penalty=['l1', 'l2'],
                           C=[1e-3, 1e-2, 1e-1, 1.0, 1e1, 1e2], nthread=4, calibrate=True, cv=False, Nboot=100):
        L = [0] * Nboot
        for i in range(Nboot):
            Ind = np.random.choice(len(Ytrain), len(Ytrain), True)
            Xt = Xtrain[Ind, :]
    ...
    # self.calculta_UCB_clipped_together(self, prun, Rrun, t, mrun, Plist, DivM, self.C2)
    ...
    def run_algorithm(self):
        ...
        Tcurr = T0 + 1
        Lxgb = self.bootstrap(Xrun, Yrun.reshape(len(Yrun)), weights=(Rrun / prun).reshape(len(Yrun)), cvbreak=None,
                              max_depths=self.max_depths, n_estimators=self.n_estimators,
                              colsample_bytrees=self.colsample_bytrees, nthread=self.nthread, calibrate=calibrate,
                              cv=False, Nboot=self.initial_model[0])
        Llogistic = self.bootstrap_logistic(Xrun, Yrun.reshape(len(Yrun)), weights=(Rrun / prun).reshape(len(Yrun)),
                                            cvbreak=None, penalty=self.penalty, C=self.C, nthread=self.nthread,
                                            calibrate=calibrate, cv=False, Nboot=self.initial_model[1])
        ...
        while t < N - 1:
            ...
            Lxgb = self.bootstrap(Xrun, Yrun.reshape(len(Yrun)), weights=(Rrun / prun).reshape(len(Yrun)), cvbreak=None,
                                  max_depths=self.max_depths, n_estimators=self.n_estimators,
                                  colsample_bytrees=self.colsample_bytrees, nthread=self.nthread, calibrate=calibrate,
                                  cv=False, Nboot=self.model_ratio[0])
            Llogistic = self.bootstrap_logistic(Xrun, Yrun.reshape(len(Yrun)), weights=(Rrun / prun).reshape(len(Yrun)),
                                                cvbreak=None, penalty=self.penalty, C=self.C, nthread=self.nthread,
                                                calibrate=calibrate, cv=False, Nboot=self.model_ratio[1])
            oldlen = len(Mlist)
            ...
            for i in range(oldlen, len(Mlist)):
                try:
                    ...
                    X = Mlist[i].predict_proba(Xrun)[0][(np.arange(t - 1), (Yrun - 1).reshape(1, t - 1))].reshape(t - 1,
                                                                                                                  1)
                Plist = np.hstack([Plist, X])
            ...
            self.Mlist = Mlist
        print
        'Achieved Mean Progressive Validation Accuracy: ' + str(np.mean(Rrun))
