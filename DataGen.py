    ...
    def gen_Y(self):
        ...
        if self.Mode == 'easy':
            ...
            Y = 5*np.array(np.sin(U2 * coef_u2y)) + \
                ...
                10*np.sin(np.array(np.power(np.abs(Z * coef_zy), 2))) * 1*np.array(2 * np.array(X_obs.T) - 1)
            ...
            Y_intv = 5*np.array(np.sin(U2 * coef_u2y)) + \
                     ...
                     10*np.sin(np.array(np.power(np.abs(Z * coef_zy),2) )) * 1*np.array(2 * np.array(X_intv.T) - 1)
