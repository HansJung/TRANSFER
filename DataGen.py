    ...
    def gen_Y(self):
        ...
        if self.Mode == 'easy':
            ...
            Y = 1*np.array(np.sin(U2 * coef_u2y)) + \
                ...
                1.5*np.sin(np.array(np.power(np.abs(Z * coef_zy), 0.3))) * 1*np.array(2 * np.array(X_obs.T) - 1)
            ...
            Y_intv = 1*np.array(np.sin(U2 * coef_u2y)) + \
                     ...
                     1.5*np.sin(np.array(np.power(np.abs(Z * coef_zy),0.3) )) * 1*np.array(2 * np.array(X_intv.T) - 1)
