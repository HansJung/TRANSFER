...
class DataGen(object):
    ...
    def gen_Y(self):
        ...
        if self.Mode == 'easy':
            ...
            Y = 1.5*np.array(np.sin(U2 * coef_u2y)) + \
                np.array(-1 * np.array(1 * np.power(U3 * coef_u3y, 1)) * 1*
                         (2*np.array(X_obs.T)-1)) + \
                0.5*np.sin(np.array(np.power(np.abs(Z * coef_zy), 0.5))) * 1*np.array(2 * np.array(X_obs.T) - 1)
            ...
            #
            Y_intv = 1.5*np.array(np.sin(U2 * coef_u2y)) + \
                     np.array(-1 * np.array(1* np.power(U3 * coef_u3y,1)) * 1*
                              (2*np.array(X_intv.T)-1)) + \
                     0.5*np.sin(np.array(np.power(np.abs(Z * coef_zy),0.5) )) * 1*np.array(2 * np.array(X_intv.T) - 1)
