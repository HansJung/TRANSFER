...
class DataGen(object):
    ...
    def __init__(self,D,N,Ns,Mode, seed_num):
    ...
    def gen_X(self):
        ...
        if self.Mode == 'easy':
            ...
            X_obs = sp.expit( np.power( np.abs(3*U1 * coef_u1x), 1)  - np.power(U3 * coef_u3x , 2) + 3*(np.exp(Z * coef_xz)) )
    ...
    def gen_Y(self):
        ...
        if self.Mode == 'easy':
            ...
            Y = -2*np.array(np.sin(U2 * coef_u2y)) + \
                ...
                2*np.sin(np.array(np.power(np.abs(Z * coef_zy), 0.2))) * 1*np.array(2 * np.array(X_obs.T) - 1)
            ...
            Y_intv = -2*np.array(np.sin(U2 * coef_u2y)) + \
                     ...
                     2*np.sin(np.array(np.power(np.abs(Z * coef_zy),0.2) )) * 1*np.array(2 * np.array(X_intv.T) - 1)
